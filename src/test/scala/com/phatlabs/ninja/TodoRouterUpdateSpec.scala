...
import com.phatlabs.ninja.model.{ApiError, Todo, UpdateTodo}
import com.phatlabs.ninja.repository.InMemoryTodoRepository
import org.scalatest.{Matchers, WordSpec}

class TodoRouterUpdateSpec
    extends WordSpec
    with Matchers
  ...
  "A TodoRouter" should {
    ...
    "update a todo with valid data" in {
      ...
      Put(s"/todos/$todoId", testUpdateTodo) ~> Route.seal(router.route) ~> check {
    ...
    "return not found with non existent todo" in {
      ...
      Put("/todos/1", testUpdateTodo) ~> Route.seal(router.route) ~> check {
    ...
    "not update a todo with invalid data" in {
      ...
      Put(s"/todos/$todoId", testUpdateTodo.copy(title = Some(""))) ~> Route
        .seal(router.route) ~> check {
